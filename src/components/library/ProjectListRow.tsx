  ...
  return (
    <div className="flex w-full items-center gap-5 border-b border-black/[0.06] py-[17px] first:pt-0 last:border-b-0">
      ...
      <div className="flex w-[75px] shrink-0 flex-col items-end gap-2">
        {project.isPro && (
          <span className="rounded-[14px] bg-brand/90 px-1.5 py-0.5 text-[10px] font-semibold tracking-[0.5px] text-white">
            PRO
          </span>
        )}
        <Link
          href={`/library/${project.slug}`}
          className="mt-auto flex items-center gap-1.5 rounded-2xl bg-brand px-3 py-2 text-xs font-semibold text-white hover:bg-brand/90"
        >
          Start
        </Link>
      </div>
