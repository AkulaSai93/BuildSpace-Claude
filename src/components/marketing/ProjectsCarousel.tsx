  ...
  return (
    <section id="projects" className="mx-auto max-w-7xl px-6 py-14">
      ...
      <div ref={trackRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 [scrollbar-width:none]">
        {featured.map((p, i) => (
          <Reveal key={p.slug} direction="up" delay={i * 100} className="shrink-0 snap-start">
            <Link
              href="/dashboard"
              className="group block w-[300px] overflow-hidden rounded-xl border border-black/[0.08] bg-white transition-all duration-300 hover:-translate-y-1 hover:border-black/[0.16]"
            >
